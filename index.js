...
const CHECK_INTERVAL = Number(process.env.CHECK_INTERVAL || 10000); // 10 sec
...
async function scanToken(symbol, token) {
  ...
  for (const acc of accounts.value) {
    const tokenAcc = new PublicKey(acc.pubkey);
    ...
    for (const s of sigs) {
      ...
      for (const ix of instructions) {
        if (ix.program === "spl-token" && ix.parsed?.type === "transfer") {
          ...
          if (dest !== tokenAcc.toString()) continue;
...
async function loop() {
  ...
  try {
    ...
    // Cleanup seenTx to prevent memory overflow
    if (seenTx.size > 5000) {
      const keys = Array.from(seenTx);
      for (let i = 0; i < keys.length / 2; i++) seenTx.delete(keys[i]);
    }
  } catch (e) {
...
/* ================= TEST ================= */
bot.onText(/\/test_sol/, () => enqueueAlert(5, "SOL", "test_tx_sol"));
bot.onText(/\/test_usdt/, () => enqueueAlert(500, "USDT", "test_tx_usdt"));
bot.onText(/\/test_usdc/, () => enqueueAlert(1000, "USDC", "test_tx_usdc"));